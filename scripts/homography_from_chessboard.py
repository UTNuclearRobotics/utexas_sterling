...
class HomographyFromChessboardImage:
    def __init__(self, image, cb_rows, cb_cols):
        ...
        self.R_ccw = np.array([[0, -1, 0], [1, 0, 0], [0, 0, 1]], dtype=np.float32) # Calibrated homography may be incorrect direction if square chessboard, rotate it if thats the case
        self.R_cw = np.array([[0, 1, 0], [-1, 0, 0], [0, 0, 1]], dtype=np.float32) # Calibrated homography may be incorrect direction if square chessboard, rotate it if thats the case
        self.R_180 = np.array([[-1, 0, 0], [0, -1, 0], [0, 0, 1]], dtype=np.float32)  # 180-degree rotation matrix
        #self.H = self.H @ self.R_ccw
    ...
    def validate_homography(self):
        ...
        while keepRunning:
            if counter % 4 == 0:
                ...
                # Compute BEV as in plot_BEV_chessboard
                ...
                rend_image = cv2.warpPerspective(image, np.linalg.inv(H), dsize=dimensions)
                cv2.setWindowTitle("Chessboard", "Bird's Eye View (BEV) Chessboard")
    ...
    def save_homography_to_yaml(self, filepath):
        ...
        # Convert self.cb_tile_width to a float
        cb_tile_width = int(self.cb_tile_width/0.1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--validate", "-v", action="store_true", help="Visualize chessboard points on the image.")
    ...
    #K, K_inv = get_homography_params().camera_intrinsics()
    #dist = get_homography_params().distortion_coefs()

    #undistort_img = undistort_image(image, K, dist, crop=True)

    # Optional: Save the undistorted image for inspection
    #output_path = os.path.join(os.path.dirname(args.image_path), 'undistorted_image.jpg')
    #cv2.imwrite(output_path, undistort_img)
    ...
    yaml_filepath = os.path.join(image_dir, yaml_filename)

    # Pass the undistorted image to HomographyFromChessboardImage
    homography_processor = HomographyFromChessboardImage(
        image=image,  # Use undistorted image
        cb_rows=args.rows,
        cb_cols=args.cols
    )

    print(f"cb_tile_width: {homography_processor.cb_tile_width} pixels")

    if args.validate:
        homography_processor.validate_homography()
